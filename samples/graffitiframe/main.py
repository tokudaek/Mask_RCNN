...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 main.py train --dataset=/path/to/graffiti/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 main.py train --dataset=/path/to/graffiti/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 main.py train --dataset=/path/to/graffiti/dataset --weights=imagenet
...
def batch_img(model, imdir, outdir, reverse=False, shuffle=False):
    print('Batch img mode')
    import matplotlib.pyplot as plt
    from mrcnn import visualize

    aux = os.listdir(imdir)
    if reverse:
        files = reversed(aux)
    elif shuffle:
